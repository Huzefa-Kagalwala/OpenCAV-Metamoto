...
FLOAT UtmConverter::DegToRad(FLOAT deg) {
  return (deg / 180.0 * pi);
}
...
FLOAT UtmConverter::RadToDeg(FLOAT rad) {
  return (rad / pi * 180.0);
}
...
FLOAT UtmConverter::ArcLengthOfMeridian(FLOAT phi) {
...
FLOAT UtmConverter::UTMCentralMeridian(int zone) {
  FLOAT cmeridian;
...
FLOAT UtmConverter::FootpointLatitude(FLOAT y) {
...
void UtmConverter::MapLatLonToXY(FLOAT phi, FLOAT lambda, FLOAT lambda0, FLOAT &x, FLOAT &y) {
...
void UtmConverter::MapXYToLatLon(FLOAT x, FLOAT y, FLOAT lambda0, FLOAT& phi, FLOAT& lambda) {
...
int UtmConverter::LatLonToUTMXY(FLOAT lat, FLOAT lon, int zone, FLOAT& x, FLOAT& y) {
  if ((zone < 1) || (zone > 60))
...
// Returns:
// The function does not return a value.
void UtmConverter::UTMXYToLatLon(FLOAT x, FLOAT y, int zone, bool southhemi, FLOAT& lat, FLOAT& lon) {
  FLOAT cmeridian;
